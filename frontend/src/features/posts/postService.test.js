...
describe('postService', () => {
  ...
  it('getPostsByTag drops the leading # and encodes the tag', async () => {
    ...
    expect(axiosInstance.get).toHaveBeenCalledWith('/api/posts?tag=hello%20world&limit=3&offset=6')
  })
  ...
  it('reactToComment sends the value to the comment url', async () => {
    ...
    expect(axiosInstance.post).toHaveBeenCalledWith('/api/posts/1/comments/c2/react', { value: -1 })
  })

  it('createComment posts the content', async () => {
    axiosInstance.post.mockResolvedValue({ data: { id: 'c1' } })
    await createComment('1', 'nice')
    expect(axiosInstance.post).toHaveBeenCalledWith('/api/posts/1/comments', { content: 'nice' })
  })
  ...
  it('updateComment puts the new content', async () => {
    ...
    expect(axiosInstance.put).toHaveBeenCalledWith('/api/posts/1/comments/c2', { content: 'edit' })
  })
